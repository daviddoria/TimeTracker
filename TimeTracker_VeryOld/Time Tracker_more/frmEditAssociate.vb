...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.lblSelect.Size = New System.Drawing.Size(110, 13)
        ...
        Me.ClientSize = New System.Drawing.Size(576, 261)
        Me.ControlBox = False
        Me.Controls.AddRange(New System.Windows.Forms.Control() {Me.lblName, Me.btnDelete, Me.lstType, Me.lblType, Me.btnQuit, Me.txtPIN, Me.lblPIN, Me.btnSave, Me.lblSelect, Me.lstNameSelect, Me.txtVacation, Me.Label1, Me.txtSick, Me.lblSick, Me.txtStartingDate, Me.lblStartDate, Me.txtSocial, Me.lblSocial, Me.lblNameTag})
...
#End Region
    ...
    Private Sub lstNameSelect_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lstNameSelect.SelectedIndexChanged
        ...
        If Not lstNameSelect.SelectedIndex = -1 Then
            ...
            If GetAssociateData(lstNameSelect.SelectedItem, EditAssociate) = False Then
                MsgBox("There was a problem with the associates data file.")
            Else
                lblName.Text = EditAssociate.Name
                txtSocial.Text = EditAssociate.Social
                txtPIN.Text = EditAssociate.Pin

                If EditAssociate.Type = "m" Then
                    lstType.SelectedItem = "Manager"
                ElseIf EditAssociate.Type = "t" Then
                    lstType.SelectedItem = "Timed"
                ElseIf EditAssociate.Type = "c" Then
                    lstType.SelectedItem = "Commissioned"
                Else
                    MsgBox("There is a problem with the AssociateType line in " & EditAssociate.Name & "'s flie")
                End If
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        CurrentType = lstType.SelectedItem
        If CurrentType = "Manager" Then
            CurrentType = "m"
        ElseIf CurrentType = "Timed" Then
            CurrentType = "t"
        ElseIf CurrentType = "Commissioned" Then
            CurrentType = "c"
        End If
        ...
        AssociateToUpdate.Type = CurrentType
        ...
        UpdateDataFile(AssociateToUpdate)
        
        'if the type has changed, update the 2 files (timed associates.txt and commissioned associates.txt)
        If Not CurrentType = EditAssociate.Type Then

            If EC = True Then
                MsgBox("Type changed!")
            End If

            If CurrentType = "m" Or CurrentType = "c" Then
                'delete their time file because they are no longer timed
                System.IO.File.Delete("c:\time tracker\associates\" & lblName.Text & ".txt")
            Else
                'create their time file now that they are not commissioned
                Dim CreateTimeFileStream As New System.IO.FileStream("c:\time tracker\associates\" & lblName.Text & ".txt", System.IO.FileMode.Create)
                Dim CreateTimeFileWriter As New System.IO.StreamWriter(CreateTimeFileStream)
                'do nothing, just create the file
                CreateTimeFileWriter.Close()
                CreateTimeFileStream.Close()

            End If
        End If


        'close the screen only if error checking is off
        If EC = False Then
            Me.Close()
        End If
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        'a counter for either case
        Dim counter As Integer = 0

        'they are a timed associate so delete their time file and remove them from the timed list
        If lstType.SelectedIndex = 1 Then
            System.IO.File.Delete("c:\time tracker\associates\" & lblName.Text & ".txt")

            If EC = True Then
                MsgBox("name removed from lstTimed and time file deleted")
            End If

        End If

        'delete their data file
        System.IO.File.Delete("c:\windows\ttdata\" & lblName.Text & ".txt")
